...
void delete_key(List* list, char key) {
	...
	cout << "For delete: " << remove_node << endl;
	delete remove_node;
...
void add_node(List* list, char str, const int& index = -1) {
	...
	int counter = 0;
	int from_tail = list->size - 1 - index;
	Node* current_node;
...
void print_list(List* list) {
	...
	while (current_node != nullptr) {
		cout << current_node->key << " (" << current_node << "); ";
		current_node = current_node->ptr_to_next_node;
	}
...
int main() {
	...
	Node test;
	cout << test.ptr_to_next_node->key;
